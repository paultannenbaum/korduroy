...
function pmxi_admin_menu() {
	...
	if (current_user_can('manage_options')) { // admin management options
		...
		/*add_submenu_page('pmxi-admin-home', __('Add-ons', 'pmxi_plugin') . ' &lsaquo; ' . __('WP All Import', 'pmxi_plugin'), __('Add-ons', 'pmxi_plugin'), 'manage_options', 'pmxi-admin-addons', array(PMXI_Plugin::getInstance(), 'adminDispatcher'));		*/
